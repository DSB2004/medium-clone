...
const SEARCH_KEYS_QUERY = `
    SELECT BP.BLOG_ID, BP.INTRO_IMAGE, BP.HEADER, BP.SEARCH_KEYS, BP.INTRO, BP.USER_NAME, BP.LIKES, BP.COMMENTS,
...
const SIMPLE_QUERY = `
    SELECT BP.BLOG_ID, BP.HEADER, BP.INTRO, BP.INTRO_IMAGE, BP.USER_NAME, BP.LIKES, BP.COMMENTS, BP.SEARCH_KEYS, 
...
const ALIAS_USER_EMAIL_QUERY = `
    SELECT BP.BLOG_ID, BP.HEADER, BP.INTRO_IMAGE, BP.INTRO, BP.USER_NAME, BP.LIKES, BP.COMMENTS, BP.SEARCH_KEYS, 
    (SELECT EXISTS (SELECT 1 FROM BLOG_LIKES BL WHERE BL.BLOG_ID = BP.BLOG_ID AND BL.USER_EMAIL = ?)) AS "LIKED_BY_YOU"
    FROM BLOG_PROFILE BP  
    WHERE BP.USER_EMAIL = ?  
    LIMIT 10 OFFSET ?;
...
const GET_BLOGS = async (req: RequestType, res: Response) => {
    ...
    try {
        ...
        if (SEARCH_KEYS && Array.isArray(SEARCH_KEYS) && SEARCH_KEYS.length > 0) {
            ...
            result = await QUERY(connection, SEARCH_KEYS_QUERY, [USER_EMAIL, SEARCH_KEYS, OFFSET]);
        } else if (ALIAS_USER_EMAIL) {
            result = await QUERY(connection, ALIAS_USER_EMAIL_QUERY, [USER_EMAIL, ALIAS_USER_EMAIL, OFFSET]);
        } else {
            result = await QUERY(connection, SIMPLE_QUERY, [USER_EMAIL, OFFSET]);
        }
